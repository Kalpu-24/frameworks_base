...
class LockScreenWidgetsController(
    ...
    internal val statusBarStateController: StatusBarStateController,
    ...
    private fun addListeners() {
        ...
        addListener(
            key = "statusBarStateListener",
            register = {
                statusBarStateController.addCallback(callbacks.statusBarStateListener)
                callbacks.statusBarStateListener.onDozingChanged(statusBarStateController.isDozing)
            },
            unregister = { statusBarStateController.removeCallback(callbacks.statusBarStateListener) }
        )
    ...
    private fun maybeKeyguardDismiss() {
        stopListening()
        scope.launch {
            delay(500)
            startListening()
    ...
    override fun onPrimaryBouncerShowingChanged(showing: Boolean) {
        if (showing) maybeKeyguardDismiss()
    }

    override fun onKeyguardGoingAwayChanged(goingAway: Boolean) {
        if (goingAway) maybeKeyguardDismiss()
    }

    override fun onKeyguardFadingAwayChanged(fadingAway: Boolean) {
        if (fadingAway) maybeKeyguardDismiss()
    }
}
